...
import net.northking.cloudtest.service.GenExecuteChartWeekService;
 ...
 */
@Service
public class GenExecuteChartWeekServiceImpl implements GenExecuteChartWeekService{


    private final static Logger logger = LoggerFactory.getLogger(GenExecuteChartWeekServiceImpl.class);
     ...
     */
    @Override
    public int GenExecuteWeekChart(String proId) throws Exception {
        ...
        ResultInfo<QualityReportDTO> res = qualityReportFeignClient.queryTestExecuteQualityReport(qualityReportQuery);
        if (res != null && res.getData() != null) {
            WordUtil wordUtil = new WordUtil();
            Map<String, Object> dataMap = new HashMap<String, Object>();
            dataMap.put("title", "用例执行完成情况");
            List<String> xAxis = res.getData().getxAxis();
            List<String> yAxis = res.getData().getyAxis();
            ...
            wordUtil.createWord("execute.json", preFilePath + qualityReportQuery.getProId() + "/json/executeByUserAndWeek.json", dataMap);
            String progressCmd = "node " + outputJsPath + " "+preFilePath + qualityReportQuery.getProId() + "/json/executeByUserAndWeek.json " + preFilePath + qualityReportQuery.getProId() + "/images/executeByUserAndWeek.png";
            try {
        ...
        query.setType("M");
        ResultInfo<QualityReportDTO> result = qualityReportFeignClient.queryTestExecuteQualityReport(query);
        if (result != null && result.getData() != null) {
            WordUtil wordUtil = new WordUtil();
            Map<String, Object> dataMap = new HashMap<String, Object>();
            dataMap.put("title", "用例执行完成情况");
            List<String> xAxis = result.getData().getxAxis();
            List<String> yAxis = result.getData().getyAxis();
            ...
            dataMap.put("yAxis7", countNum(yAxis, "6"));
            dataMap.put("yAxisTotal", counttotal(yAxis));
            wordUtil.createWord("execute.json", preFilePath + qualityReportQuery.getProId() + "/json/executeByModuleAndWeek.json", dataMap);
            String progressCmd = "node " + outputJsPath + " "+preFilePath + qualityReportQuery.getProId() + "/json/executeByModuleAndWeek.json " + preFilePath + qualityReportQuery.getProId() + "/images/executeByModuleAndWeek.png";
            try {
                Process p = run.exec(progressCmd);
            } catch (Exception e) {
                e.printStackTrace();
      ...
      /*  SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        ...
        if (list.size() > 0) {
            ...
            for (CltRound cltRound : list
                ...
                System.out.println("人员轮次id" + roundId);
                ...
                if (testCaseChartDate.size() > 0) {
                    for (TestCaseExecuteReportDTO testCaseExecuteReportDTO : testCaseChartDate
                        ...
                        System.out.println("人员名称为" + xAxis.toString());
                    }
                }
                dataMap.put("title", "用例执行完成情况");
                dataMap.put("xAxis", JSONObject.toJSONString(xAxis));
                ...
                i++;
                wordUtil.createWord("execute.json", preFilePath + proId + "/json/executeAllByUserAndRound" + i + ".json", dataMap);
                String progressCmd = "node " + outputJsPath + " " + preFilePath + proId + "/json/executeAllByUserAndRound" + i + ".json " + preFilePath + proId + "/images/executeAllByUserAndRound" + i + ".png";
                try {
                    Process p = run.exec(progressCmd);
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        //模块
            ...
            if (list.size() > 0) {
                ...
                for (CltRound cltRound : list
                    ...
                    String roundId = cltRound.getRoundId();
                    System.out.println("模块轮次id" + roundId);
                    ...
                    if (testCaseChartDate.size() > 0) {
                        for (TestCaseExecuteReportDTO testCaseExecuteReportDTO : testCaseChartDate
                            ...
                            yAxis6.add(testCaseExecuteReportDTO.getExecute5());
                            yAxis7.add(testCaseExecuteReportDTO.getExecute6());
                            System.out.println("模块名称为" + xAxis.toString());
                        }
                    }
                    dataMap.put("title", "用例执行完成情况");
                    dataMap.put("xAxis", JSONObject.toJSONString(xAxis));
                    ...
                    dataMap.put("yAxis7", JSONObject.toJSONString(yAxis7));
                    j++;
                    wordUtil.createWord("execute.json", preFilePath + proId + "/json/executeAllByModuleAndRound"+j+".json", dataMap);
                    String progressCmd = "node " + outputJsPath + " " + preFilePath + proId + "/json/executeAllByModuleAndRound"+j+".json " + preFilePath + proId + "/images/executeAllByModuleAndRound"+j+".png";
                    try {
                        Process p = run.exec(progressCmd);
                    } catch (Exception e) {
                        e.printStackTrace();
